...
threads-bot と同じ形式。違うのは文字数の数え方だけ（X は日本語を 2 として数える）。
"""
...
def _parse_item(raw: dict, *, line_number: int, tz: ZoneInfo) -> QueueItem:
    ...
    if length > MAX_TEXT_LENGTH:
        raise QueueError(
            f"{where}: text が上限を超えています（{length} / {MAX_TEXT_LENGTH}）。"
            "X は日本語を 1 文字 2 として数えます（日本語なら 140 字まで）。"
        )
    ...
    for index, part in enumerate(thread, start=2):
        part_length = weighted_length(part)
        if part_length > MAX_TEXT_LENGTH:
            raise QueueError(
